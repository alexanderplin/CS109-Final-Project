			...
			for nodes in leaf_nodes:
				for trees in n_trees:
					for depth in depths:
						...
						if np.mean(average_score) >= best_avg and average_score[2] > 0.5 and average_score[1] > 0.5:
							best_avg = np.mean(average_score)
							best_trees_avg = trees
							best_depth_avg = depth
							best_nodes_avg = nodes
							best_pos = average_score[2]
							best_neg = average_score[1]
			result += [(name,n,pca_n, best_trees_avg, best_depth_avg, best_nodes_avg, best_avg, best_pos, best_neg)]
	return result
...
if __name__ == '__main__':
	...
	for date in filtered_df['pub_date']:
		...
		threshold = 0.005 * price_before[0]
